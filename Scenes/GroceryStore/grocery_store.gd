...
func get_profit() -> float:
	var sold_food = min(get_demand(), get_supply())
	return clamp((crisis.multiplier + 0.5 * resilience_score), 0, 1) * (sold_food * get_price() - wages * houses.size()) * income_bonus
...
func get_supply() -> float:
	return food_intake
	#return food_intake + crisis.gs_supply_adjust * (1 - resilience_score * resilience_score)
	#return food_intake * (1 - (1 - crisis.import_supply_mult) * (1 - resilience_score))

func get_price() -> float:
	return price
	#return income_bonus * (price + crisis.gs_income_adjust * (1 - resilience_score * resilience_score))
